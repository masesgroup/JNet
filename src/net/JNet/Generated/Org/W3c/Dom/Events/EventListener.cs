...
namespace Org.W3c.Dom.Events
{
    ...
    public partial class EventListener : Org.W3c.Dom.Events.IEventListener
    {
        ...
        {
            _HandleEventEventHandlerIndex = AddEventHandler("handleEvent", new global::System.EventHandler<CLRListenerEventArgs<MASES.JNet.Specific.JNetEventData>>(HandleEventEventHandler));
            _hasOverrideHandleEvent = MASES.JNet.Specific.JNetEventResult.GetMethodIsOverridden(listenerRuntimeType, nameof(HandleEvent), typeof(Org.W3c.Dom.Events.Event));

        }
        /// <inheritdoc/>
        protected virtual bool ListenerShallManageEventHandlers(int eventIndex)
        {
            if (eventIndex == _HandleEventEventHandlerIndex) return HandleEventEventOverridden();
        ...
        /// <summary>Index assigned by <see cref="AddEventHandler"/> for the <see cref="HandleEvent"/> event. Used for zero-cost index-based filtering in <see cref="ListenerShallManageEvent(int)"/>.</summary>
        int _HandleEventEventHandlerIndex = 0;

        /// <summary><see langword="true"/> if the user has overridden <see cref="HandleEvent"/> in a subclass. Cached at construction to avoid per-event reflection.</summary>
        bool _hasOverrideHandleEvent;
        ...
        void HandleEventEventHandler(object sender, CLRListenerEventArgs<MASES.JNet.Specific.JNetEventData> data)
        {
            if (!HandleEventEventOverridden()) // guard: no handler registered — return without reading data.
            ...
            data.EventData.HasOverride = HandleEventEventOverridden();
        }
