...
namespace Java.Awt.EventNs
{
    ...
    public partial class FocusAdapter : MASES.JCOBridge.C2JBridge.JVMBridgeListener
    {
        ...
        /// <item><see cref="_hasFocusAdapterSecondGate"/> — returns <see langword="true"/> if the second gate is overridden, ensuring all events reach it.</item>
        ...
        protected override bool ListenerShallManageEvent(int eventIndex)
        {
            ...
            return _hasFocusAdapterSecondGate || ListenerShallManageEventIndexWithData != null || ListenerShallManageEventNameWithData != null;
        }
    ...
    /// <summary>
    /// Concrete CLR representation of <see cref="FocusAdapter"/> returned by the JVM.
    /// </summary>
    /// <remarks>
    ...
    /// <para>Do not use this class directly to register event handlers — use <see cref="FocusAdapter"/> instead.</para>
    /// </remarks>
    ...
    public partial class FocusAdapter
    {
        ...
        /// </summary>
        public FocusAdapter() : base(JVMBridgeBase.InitializerInstance)
        {
            InvokeInitializer();
        }
        ...
        {
            _FocusGainedEventHandlerIndex = AddEventHandler("focusGained", new global::System.EventHandler<CLRListenerEventArgs<MASES.JNet.Specific.JNetEventData>>(FocusGainedEventHandler));
            ...
            _FocusLostEventHandlerIndex = AddEventHandler("focusLost", new global::System.EventHandler<CLRListenerEventArgs<MASES.JNet.Specific.JNetEventData>>(FocusLostEventHandler));
        ...
        {
            if (eventIndex == _FocusGainedEventHandlerIndex) return FocusGainedEventOverridden();
            if (eventIndex == _FocusLostEventHandlerIndex) return FocusLostEventOverridden();
        ...
        /// <summary>Index assigned by <see cref="AddEventHandler"/> for the <see cref="FocusGained"/> event. Used for zero-cost index-based filtering in <see cref="ListenerShallManageEvent(int)"/>.</summary>
        int _FocusGainedEventHandlerIndex = 0;

        /// <summary><see langword="true"/> if the user has overridden <see cref="FocusGained"/> in a subclass. Cached at construction to avoid per-event reflection.</summary>
        ...
        void FocusGainedEventHandler(object sender, CLRListenerEventArgs<MASES.JNet.Specific.JNetEventData> data)
        {
            if (!FocusGainedEventOverridden()) // guard: no handler registered — return without reading data.
            ...
            data.EventData.HasOverride = FocusGainedEventOverridden();
        }
        ...
        /// <summary>Index assigned by <see cref="AddEventHandler"/> for the <see cref="FocusLost"/> event. Used for zero-cost index-based filtering in <see cref="ListenerShallManageEvent(int)"/>.</summary>
        int _FocusLostEventHandlerIndex = 0;

        /// <summary><see langword="true"/> if the user has overridden <see cref="FocusLost"/> in a subclass. Cached at construction to avoid per-event reflection.</summary>
        ...
        void FocusLostEventHandler(object sender, CLRListenerEventArgs<MASES.JNet.Specific.JNetEventData> data)
        {
            if (!FocusLostEventOverridden()) // guard: no handler registered — return without reading data.
                                                                                  // Normally unreachable if ListenerShallManageEvent filtered correctly,
                                                                                  // but protects against user overrides of ListenerShallManageEvent that bypass the check.
            {
                // data.EventData.HasOverride is false by default — JVM receives no return value.
            ...
            methodToExecute.Invoke(data.EventData.GetAt<Java.Awt.EventNs.FocusEvent>(0));
            data.EventData.HasOverride = FocusLostEventOverridden();
        }
