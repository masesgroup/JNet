...
namespace Java.Nio
{
    public partial class ShortBuffer
    {
        ...
        /// so capacity will be memory aligned and the limit of java.nio.ShortBuffer will be current size of <paramref name="data"/>
        /// </param>
        ...
        public static ShortBuffer From(short[] data, bool arrangeCapacity = true, int timeToLive = System.Threading.Timeout.Infinite)
        {
            try
            {
                return data.DirectBufferWithWrap<short, ShortBuffer>(arrangeCapacity, timeToLive);
            }
            catch (UnsupportedOperationException) { }
            catch (System.NotSupportedException) { }
        ...
        /// to be populated via Stream-based APIs and then passed to <see cref="From(JCOBridgeSharedBufferStream{short})"/>.
        /// </summary>
        /// <param name="capacity">The plausible number of elements of <see langword="short"/> type to be written into the stream; the byte size is computed as <c>capacity * sizeof(<see langword="short"/>)</c>.
        ...
        /// </param>
        /// <returns>A pooled instance of <see cref="JCOBridgeSharedBufferStream{T}"/> ready to be written via Stream-based APIs and then passed to <see cref="From(JCOBridgeSharedBufferStream{short})"/>.</returns>
        /// <exception cref="ArgumentOutOfRangeException">Thrown when <paramref name="capacity"/> is zero or any positive value that, once multiplied by <c>sizeof(<see langword="short"/>)</c>, overflows a <see cref="long"/>.</exception>
        /// <remarks>
        ...
        /// <see cref="ShortBuffer"/> created by <see cref="From(JCOBridgeSharedBufferStream{short})"/>.
        /// </remarks>
        public static JCOBridgeSharedBufferStream<short> Rent(long capacity = -1)
        {
            return JCOBridge.Global.JVM.Rent<short>(capacity);
        }

        /// <summary>
        /// Creates a new <see cref="ShortBuffer"/> in the JVM which shares the memory of <paramref name="stream"/>.
        ...
        /// to be used directly within the JVM from a <see cref="ShortBuffer"/>.</param>
        /// <returns>A new instance of <see cref="ShortBuffer"/> holding the memory of <paramref name="stream"/> shared with the <see cref="ShortBuffer"/>.</returns>
        /// <remarks>
        /// The memory associated to <paramref name="stream"/> will be retained until the JVM reference of the newly created <see cref="ShortBuffer"/> is garbage collected.
        ...
        /// <see cref="ShortBuffer"/>, i.e. when the <see cref="ShortBuffer"/> has been fully consumed by the JVM.
        /// A direct call to <see cref="IDisposable.Dispose"/> on the returned <see cref="ShortBuffer"/> is therefore a no-op; do not attempt to manually dispose <paramref name="stream"/> after passing it to this method.
        /// </para>
        /// <para>
        ...
        /// <exception cref="NotSupportedException">Thrown when the JVM is unable to generate a <see cref="ShortBuffer"/> instance.</exception>
