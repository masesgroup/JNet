...
namespace Java.Nio
{
    public partial class IntBuffer
    {
        ...
        /// </summary>
        public static implicit operator JCOBridgeDirectBuffer<int>(IntBuffer t) => t.ToDirectBuffer(true);
        /// <summary>
        ...
        public int[] ToArray(bool bypassDirectConvert = false)
        {
            if (!bypassDirectConvert)
            {
                try
                {
                    return ToDirectBuffer(true).ToArray<int>();
                }
        ...
        /// so capacity will be memory aligned and the limit of java.nio.IntBuffer will be current size of <paramref name="data"/>
        /// </param>
        ...
        /// to be populated via Stream-based APIs and then passed to <see cref="From(JCOBridgeSharedBufferStream{int})"/>.
        /// </summary>
        /// <param name="capacity">The plausible number of elements of <see langword="int"/> type to be written into the stream; the byte size is computed as <c>capacity * sizeof(<see langword="int"/>)</c>.
        ...
        /// </param>
        /// <returns>A pooled instance of <see cref="JCOBridgeSharedBufferStream{T}"/> ready to be written via Stream-based APIs and then passed to <see cref="From(JCOBridgeSharedBufferStream{int})"/>.</returns>
        /// <exception cref="ArgumentOutOfRangeException">Thrown when <paramref name="capacity"/> is zero or any positive value that, once multiplied by <c>sizeof(<see langword="int"/>)</c>, overflows a <see cref="long"/>.</exception>
        /// <remarks>
        ...
        /// <see cref="IntBuffer"/> created by <see cref="From(JCOBridgeSharedBufferStream{int})"/>.
        /// </remarks>
        public static JCOBridgeSharedBufferStream<int> Rent(long capacity = -1)
        {
            return JCOBridge.Global.JVM.Rent<int>(capacity);
        }

        /// <summary>
        /// Creates a new <see cref="IntBuffer"/> in the JVM which shares the memory of <paramref name="stream"/>.
        ...
        /// to be used directly within the JVM from a <see cref="IntBuffer"/>.</param>
        /// <returns>A new instance of <see cref="IntBuffer"/> holding the memory of <paramref name="stream"/> shared with the <see cref="IntBuffer"/>.</returns>
        /// <remarks>
        /// The memory associated to <paramref name="stream"/> will be retained until the JVM reference of the newly created <see cref="IntBuffer"/> is garbage collected.
        ...
        /// <see cref="IntBuffer"/>, i.e. when the <see cref="IntBuffer"/> has been fully consumed by the JVM.
        /// A direct call to <see cref="IDisposable.Dispose"/> on the returned <see cref="IntBuffer"/> is therefore a no-op; do not attempt to manually dispose <paramref name="stream"/> after passing it to this method.
        /// </para>
        /// <para>
        ...
        /// <exception cref="NotSupportedException">Thrown when the JVM is unable to generate a <see cref="IntBuffer"/> instance.</exception>
        public static IntBuffer From(JCOBridgeSharedBufferStream<int> stream)
        {
            var buf = JCOBridge.Global.JVM.NewDirectBuffer(stream);
            return JVMBridgeBase.WrapsDirect<IntBuffer>(buf.DisableCleanupAndReturn());
        }
        ...
        /// <returns>The <see cref="JCOBridgeDirectBuffer{T}"/> associated to this <see cref="IntBuffer"/> instance</returns>
        /// <remarks>
        ...
        public JCOBridgeDirectBuffer<int> ToDirectBuffer(bool rewind)
        {
            if (rewind)
            {
