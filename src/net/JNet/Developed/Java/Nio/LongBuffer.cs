...
namespace Java.Nio
{
    public partial class LongBuffer
    {
        ...
        /// </summary>
        public static implicit operator JCOBridgeDirectBuffer<long>(LongBuffer t) => t.ToDirectBuffer(true);
        /// <summary>
        /// Converts an instance of <see cref="long"/> array into <see cref="LongBuffer"/> using the default parameters of <see cref="From(long[], bool, int)"/>
        /// </summary>
        ...
        public long[] ToArray(bool bypassDirectConvert = false)
        {
            if (!bypassDirectConvert)
            {
                try
                {
                    return ToDirectBuffer(true).ToArray<long>();
                }
        ...
        public static LongBuffer From(long[] data, bool arrangeCapacity = true, int timeToLive = System.Threading.Timeout.Infinite)
        {
            try
            {
                return data.DirectBufferWithWrap<long, LongBuffer>(arrangeCapacity, timeToLive);
            }
            catch (UnsupportedOperationException) { }
            catch (System.NotSupportedException) { }
        ...
        /// to be populated via Stream-based APIs and then passed to <see cref="From(JCOBridgeSharedBufferStream{long})"/>.
        /// </summary>
        /// <param name="capacity">The plausible number of elements of <see langword="long"/> type to be written into the stream; the byte size is computed as <c>capacity * sizeof(<see langword="long"/>)</c>.
        ...
        /// <exception cref="ArgumentOutOfRangeException">Thrown when <paramref name="capacity"/> is zero or any positive value that, once multiplied by <c>sizeof(<see langword="long"/>)</c>, overflows a <see cref="long"/>.</exception>
        /// <remarks>
        ...
        /// <see cref="LongBuffer"/> created by <see cref="From(JCOBridgeSharedBufferStream{long})"/>.
        /// </remarks>
        public static JCOBridgeSharedBufferStream<long> Rent(long capacity = -1)
        {
        ...
        /// <returns>A new instance of <see cref="LongBuffer"/> holding the memory of <paramref name="stream"/> shared with the <see cref="LongBuffer"/>.</returns>
        /// <remarks>
        ...
        /// A direct call to <see cref="IDisposable.Dispose"/> on the returned <see cref="LongBuffer"/> is therefore a no-op; do not attempt to manually dispose <paramref name="stream"/> after passing it to this method.
        /// </para>
        /// <para>
        ...
        public static LongBuffer From(JCOBridgeSharedBufferStream<long> stream)
        {
        ...
        public JCOBridgeDirectBuffer<long> ToDirectBuffer(bool rewind)
        {
            if (rewind)
            {
