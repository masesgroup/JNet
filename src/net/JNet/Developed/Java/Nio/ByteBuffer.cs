...
namespace Java.Nio
{
    public partial class ByteBuffer : IDisposable
    {
        ...
        /// If the user of <see cref="ByteBuffer"/> is pretty sure that the memory is no more needed from the JVM, e.g. the invoked method does not queue the <see cref="ByteBuffer"/> and its lifetime ends when the method returns, invoke <see cref="ToDirectBuffer()"/> and then <see cref="JCOBridgeDirectBuffer{T}.Dispose()"/> to immediately release unmanaged resources and free the memory
        /// </remarks>
