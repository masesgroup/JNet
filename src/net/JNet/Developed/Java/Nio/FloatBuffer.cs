...
namespace Java.Nio
{
    public partial class FloatBuffer
    {
        ...
        /// </summary>
        public static implicit operator JCOBridgeDirectBuffer<float>(FloatBuffer t) => t.ToDirectBuffer(true);
        /// <summary>
        ...
        public float[] ToArray(bool bypassDirectConvert = false)
        {
            if (!bypassDirectConvert)
            {
                try
                {
                    return ToDirectBuffer(true).ToArray<float>();
                }
        ...
        /// so capacity will be memory aligned and the limit of java.nio.FloatBuffer will be current size of <paramref name="data"/>
        /// </param>
        ...
        /// to be populated via Stream-based APIs and then passed to <see cref="From(JCOBridgeSharedBufferStream{float})"/>.
        /// </summary>
        /// <param name="capacity">The plausible number of elements of <see langword="float"/> type to be written into the stream; the byte size is computed as <c>capacity * sizeof(<see langword="float"/>)</c>.
        ...
        /// </param>
        /// <returns>A pooled instance of <see cref="JCOBridgeSharedBufferStream{T}"/> ready to be written via Stream-based APIs and then passed to <see cref="From(JCOBridgeSharedBufferStream{float})"/>.</returns>
        /// <exception cref="ArgumentOutOfRangeException">Thrown when <paramref name="capacity"/> is zero or any positive value that, once multiplied by <c>sizeof(<see langword="float"/>)</c>, overflows a <see cref="long"/>.</exception>
        /// <remarks>
        ...
        /// <see cref="FloatBuffer"/> created by <see cref="From(JCOBridgeSharedBufferStream{float})"/>.
        /// </remarks>
        public static JCOBridgeSharedBufferStream<float> Rent(long capacity = -1)
        {
            return JCOBridge.Global.JVM.Rent<float>(capacity);
        }

        /// <summary>
        /// Creates a new <see cref="FloatBuffer"/> in the JVM which shares the memory of <paramref name="stream"/>.
        ...
        /// to be used directly within the JVM from a <see cref="FloatBuffer"/>.</param>
        /// <returns>A new instance of <see cref="FloatBuffer"/> holding the memory of <paramref name="stream"/> shared with the <see cref="FloatBuffer"/>.</returns>
        /// <remarks>
        /// The memory associated to <paramref name="stream"/> will be retained until the JVM reference of the newly created <see cref="FloatBuffer"/> is garbage collected.
        ...
        /// <see cref="FloatBuffer"/>, i.e. when the <see cref="FloatBuffer"/> has been fully consumed by the JVM.
        /// A direct call to <see cref="IDisposable.Dispose"/> on the returned <see cref="FloatBuffer"/> is therefore a no-op; do not attempt to manually dispose <paramref name="stream"/> after passing it to this method.
        /// </para>
        /// <para>
        ...
        /// <exception cref="NotSupportedException">Thrown when the JVM is unable to generate a <see cref="FloatBuffer"/> instance.</exception>
        public static FloatBuffer From(JCOBridgeSharedBufferStream<float> stream)
        {
            ...
            return JVMBridgeBase.WrapsDirect<FloatBuffer>(buf.DisableCleanupAndReturn());
        }
        ...
        /// <returns>The <see cref="JCOBridgeDirectBuffer{T}"/> associated to this <see cref="FloatBuffer"/> instance</returns>
        /// <remarks>
