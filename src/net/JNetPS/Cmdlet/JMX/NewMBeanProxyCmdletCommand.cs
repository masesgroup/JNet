...
{
    [Cmdlet(VerbsCommon.New, "MBeanProxy")]
    public class NewMBeanProxyCmdletCommand : JNetPSCmdlet<JNetPSCore>
    {
        ...
        protected override void ProcessCommand()
        {
            ...
            if (Class != null)
            {
                ...
                {
                    result = Javax.Management.JMX.NewMBeanProxy(Connector.MBeanServerConnection, ObjectName, Class, true);
                }
                else
                {
                    result = Javax.Management.JMX.NewMBeanProxy(Connector.MBeanServerConnection, ObjectName, Class);
                }
            }
            else if (InterfaceName != null)
            {
                if (WithNotificationEmitter.IsPresent)
                {
                    result = Javax.Management.JMX.NewMBeanProxy(Connector.MBeanServerConnection, ObjectName, Java.Lang.Class.ForName(InterfaceName), true);
                }
                else
                {
                    result = Javax.Management.JMX.NewMBeanProxy(Connector.MBeanServerConnection, ObjectName, Java.Lang.Class.ForName(InterfaceName));
                }
            }
