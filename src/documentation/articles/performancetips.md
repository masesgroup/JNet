...
## Preallocate JVM™ objects for repeated use

When a JVM™ object (a `Java.Lang.String`, a wrapper class, a configuration object) needs to be passed to JVM™ methods many times, creating it once and reusing the instance eliminates per-call marshalling cost.

JNet wrapper classes (generated by JNetReflector or hand-written) hold a reference to the underlying JVM™ object. Passing such an instance to an `Invoke` call transfers only the native object pointer — no allocation, no conversion. The cost is essentially the same as invoking a no-argument method.

```csharp
// Slow path — marshalling on every call
for (int i = 0; i < 1_000_000; i++)
{
    _instance.Invoke("processItem", "constant-key");   // .NET string → JVM String on each call
}

// Fast path — preallocate once, pass the JNet object
using var key = new Java.Lang.String("constant-key"); // single JVM allocation
for (int i = 0; i < 1_000_000; i++)
{
    _instance.Invoke("processItem", key);              // only native pointer passed
}
```

This applies to **any JNet wrapper class**, not just `Java.Lang.String` — configuration objects, enum constants, shared keys, format patterns, or any object whose identity or content is stable across calls.

### Preallocated arrays

The same principle applies to arrays of JNet wrapper objects. Preallocating a `Java.Lang.String[]` and passing it to a JVM method avoids per-element string marshalling:

```csharp
// Preallocate the array of JNet String objects
var preallocated = new Java.Lang.String[items.Length];
for (int i = 0; i < items.Length; i++)
    preallocated[i] = new Java.Lang.String(items[i]);

try
{
    // Each call passes native references, no per-call marshalling
    _instance.Invoke("processAll", preallocated);
}
finally
{
    foreach (var s in preallocated) s?.Dispose();
}
```

### Performance impact

From BenchmarkDotNet on `ubuntu-24.04` x86-64 (.NET 10 / Temurin 25):

| Approach | 100 000 elements | Per-element |
|---|---|---|
| `int[]` | 67 µs | ~0.6 ns |
...
> [!NOTE]
> Preallocated JNet objects must be disposed when no longer needed. Use `Dispose()` explicitly or wrap in `using`. Failing to dispose leaks the underlying JVM global reference until the GC finalizes the .NET wrapper.

## Prefer primitive types at the JVM™ boundary

The cost of crossing the JVM↔CLR boundary varies dramatically by argument type. For arrays, the difference between primitive and reference types is not a constant factor — it grows linearly with element count:
...
- **Primitive arrays** (`int[]`, `long[]`, `byte[]`, `double[]`) for bulk data — transferred as contiguous memory
- **`JCOBridgeStream<T>`** for large binary payloads — zero-copy with HPA, avoids the boundary entirely
- **Preallocated JNet wrapper objects** when the same reference objects are reused across many calls
- **Avoid `.NET string[]`** in hot loops — each element requires individual Unicode conversion and JVM heap allocation
